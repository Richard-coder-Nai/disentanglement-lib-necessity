# Revisiting Disentanglement in Downstream Tasks: A Study on Its Necessity for Abstract Visual Reasoning (AAAI 2024)

This is the source code for the AAAI 2024 paper. The implementation is based on https://github.com/google-research/disentanglement_lib.

## Evaluate a representation
Use the following code to evaluate a representation.
``` bash
dlib_evaluate \
      --input_dir=$model_dir \
      --output_dir=$output_dir \
      --overwrite=1 \
      --gin_bindings="dataset.name='$dataset_name'" \
      --gin_bindings="global_stat.by='$scale'" \
      --gin_bindings="evaluation.scaler=@global_stat" \
      --gin_config="disentanglement_lib/config/abstract_reasoning_study_v1/stage1/metric_configs/${metric}.gin"
```
In the code snippet, the `global_stat` option is used to apply normalization for BYOL models. The available metrics can be found in the `disentanglement_lib/config/abstract_reasoning_study_v1/stage1/metric_configs` directory. The available datasets are *3D Shapes* and *Abstract dSprites*.

## Train a stage-2 model
Use the following code to train a WReN model.
``` bash
python reason.py \
    --output_dir $output_dir \
    --overwrite \
    --gin_config from_representation.gin \
    --reason_index $reason_index \
    --gin_bindings \
    "TwoStageModel.embedding_model_class=@HubEmbedding" \
    "TwoStageModel.rotate_embedding=0" \
    "HubEmbedding.hub_path='$hub_path'" \
    "global_stat.by='$scale'" \
...
```bash
  python reason_transformer.py \
    --output_dir abstract_dsprites_byol_reason_transformer_checkpoints/$tag \
    --overwrite \
    --gin_config from_representation_transformer.gin \
    --reason_index $reason_index \
    --gin_bindings \
    "TwoStageModel.embedding_model_class=@HubEmbedding" \
    "TwoStageModel.rotate_embedding=0" \
...
The following is the original README.md from `disentanglement_lib`, including basic installation and usage.
