...
class AbstractReasoningStudyV1(study.Study):
  ...
  def get_eval_config_files(self):
    ...
    # Due to cross validation bug we need to drop Logistic regression.
    # return list(
    #     resources.get_files_in_folder(
    #         "config/abstract_reasoning_study_v1/stage1/metric_configs/"))
    return [cfg for cfg in resources.get_files_in_folder("config/abstract_reasoning_study_v1/stage1/metric_configs/") if any([s in cfg for s in ['knn', 'logistic_regression', 'linear_regression', 'med', 'factor_vae_metric']]) ]
