...
@gin.configurable("global_stat")
def _stat_scaler(mean=gin.REQUIRED,
                       std=gin.REQUIRED,
                       by=None):
  """Get the sacaler function given mean and std.
  Args:
    mean: Numpy array of shape (z_dim,), indicating the mean of each dimension.
    std: Numpy array of shape (z_dim,), indicating the std of each dimension.
    by: Either 'std', 'mean'  or 'both' or 'l2', can be consecutive joined by '+'.

  Returns:
    A scaler function that takes representations as input.
  """
  def _single_scaler(representations, op=None):
    if not op:
      return representations
    if op == 'mean':
      return representations - np.reshape(mean, (1,-1))
    if op == 'std':
      return representations / np.reshape(std,(1,-1))
    if op == 'both':
      return (representations - np.reshape(mean, (1,-1))) / np.reshape(std,(1,-1))
    if op == 'l2':
      if tf.is_tensor(representations):
        return representations / tf.norm(representations, axis=1, keepdims=True)
      else:
        return representations / np.linalg.norm(representations, axis=1, keepdims=True)
    else:
      return representations

  def _scaler(representations):
    if not by:
      return representations
    else:
      for op in by.split('+'):
        representations = _single_scaler(representations, op=op)
      return representations
  return _scaler
...
@gin.configurable("logistic_regression")
def logistic_regression():
  """Logistic regression."""
  return linear_model.LogisticRegression()

gin.external_configurable(linear_model.LogisticRegression)
gin.external_configurable(neural_network.MLPClassifier)
