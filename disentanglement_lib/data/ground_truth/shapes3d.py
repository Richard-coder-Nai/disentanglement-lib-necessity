...
class Shapes3D(ground_truth_data.GroundTruthData):
  ...
  def __init__(self):
    self.images = np.load(os.path.join(SHAPES3D_PATH,'images.npy'), mmap_mode='r')
    labels = np.load(os.path.join(SHAPES3D_PATH, 'labels.npy'))
  ...
  def sample_observations_from_factors(self, factors, random_state):
    ...
    output = np.zeros((len(indices), 64, 64, 3), dtype=np.float32)
    output[:] = self.images[indices]
    return output
