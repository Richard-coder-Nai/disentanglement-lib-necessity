...
if __name__ == "__main__":
  ...
  model_binding_keys = ["lr/CosineScheduler.base_value",
 "OptimizedTransformer.dropout_in_last_graph_layer",
 "OptimizedTransformer.graph_mlp",
 "wd/CosineScheduler.base_value",
 "OptimizedTransformer.depth",
          ]
  model_binding_candidates = [
        ["5e-4", "6e-4", "7e-4"],# leraning rate
        ["0.5", "0.75", "0.90", "None"],# dropout in last graph layer
        ["[256, 256]", "[128, 128]", "[128, 128, 128]", "[256, 256, 256]"],# graph mlp
        ["1e-4", "1e-4", "4"],# weight decay
        ["4","6"]# depth
        ]
  # The total sample space size is 3*4*4*3*2=288
  ...
  command_args.gin_bindings.append(f"lr/CosineScheduler.final_value={float(model_binding_values[0]) / 100.}")
  command_args.gin_bindings.append(f"wd/CosineScheduler.final_value={float(model_binding_values[3]) / 100.}")
